...
export interface WithFairGardenDocsOptions {
  /**
   * Additional page extensions to support beyond the default FairGarden Docs extensions.
   * Default FairGarden Docs extensions are: ['js', 'jsx', 'md', 'mdx', 'ts', 'tsx']
   */
  ...
  /**
   * Additional Turbopack rules to merge with the default FairGarden Docs rules.
   */
  ...
  /**
   * Directory rooting FairGarden Docs's build caches and coordination state — relocate it (or point it
   ...
   * (`index-updates`, `types-index-updates`). The types socket dir (`.next/fair-garden-docs`) is separate.
   * @default '.next/cache/fairgarden-docs'
   */
  cacheDir?: string;
}

export interface FairGardenDocsMdxOptions {
  remarkPlugins?: Array<string | [string, ...any[]]>;
  rehypePlugins?: Array<string | [string, ...any[]]>;
  /**
   * Additional remark plugins to add to the default FairGarden Docs plugins
   */
  additionalRemarkPlugins?: Array<string | [string, ...any[]]>;
  /**
   * Additional rehype plugins to add to the default FairGarden Docs plugins
   */
   ...
   * Only needed when calling the plugin directly (not via withFairGardenDocs).
   */
  baseDir?: string;
  /**
  ...
  codeBlockEmphasisOptions?: TransformHtmlCodeBlockOptions;
  /**
   * Directory rooting FairGarden Docs's build caches and index markers. Indexes synced from MDX store
...
/**
 * Get default MDX options for FairGarden Docs
 */
export function getFairGardenDocsMdxOptions(
  customOptions: FairGardenDocsMdxOptions = {},
): FairGardenDocsMdxOptions {
  const {
    extractToIndex = true,
    baseDir,
  ...
  const defaultRemarkPlugins: Array<string | [string, ...any[]]> = [
    ...
    defaultInlineCodeLanguage !== undefined
      ? ['@fairgarden/docs/pipeline/transformMarkdownCode', { defaultInlineCodeLanguage }]
...
/**
 * Next.js plugin for FairGarden Docs.
 * Configures webpack loaders, turbopack rules for docs sites.
 * Use getFairGardenDocsMdxOptions() with createMDX for MDX integration.
 */
export function withFairGardenDocs(options: WithFairGardenDocsOptions = {}) {
  const {
        ...
        loaders: [
          {
            ...
            options: {
              performance,
              socketDir: '.next/fairgarden-docs',
              updateParentIndex,
              cacheDir,
              ...(codeBlockEmphasisOptions
