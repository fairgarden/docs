...
function DemoComponent() {
  ...
  const { Button } = externals['@base-ui/react/button'] || {};
...
const context = {
  externals: {
    ...
    '@base-ui/react/direction-provider': { DirectionProvider, useDirection }, // Multiple named imports
  },
};
```
...
function SafeExternalDemo() {
  ...
  const { '@base-ui/react/button': MUI = {} } = externalsContext.externals;
  const { Button } = MUI;
...
function ConditionalDemo() {
  ...
  const { Button, Typography } = externals['@base-ui/react/button'] || {};
...
The context works seamlessly with the FairGarden Docs build system:
