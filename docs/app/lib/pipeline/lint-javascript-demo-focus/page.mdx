...
Demo files in `docs/app/**/demos/**` need `@focus` comments so FairGarden Docs knows which code to emphasize. Writing these by hand is tedious and error-prone. This ESLint rule detects the preview section of each demo and auto-fixes the comments in.
...
export default [
  {
    ...
    rules: {
      'fairgarden-docs/require-demo-focus': 'error',
    },
  },
];
```
