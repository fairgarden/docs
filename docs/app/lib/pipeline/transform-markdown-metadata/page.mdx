...
import transformMarkdownMetadata from '@fairgarden/docs/pipeline/transformMarkdownMetadata';
...
When using the plugin directly (without `withFairGardenDocs`), you'll need to provide the `baseDir` for path filtering to work correctly:
...
### With withFairGardenDocs Plugin

The [`withFairGardenDocs`](../../pipeline/with-fairgarden-docs/page.mdx) Next.js plugin automatically includes this with the correct configuration:

```javascript
// next.config.js
import createMDX from '@next/mdx';
import { withFairGardenDocs, getFairGardenDocsMdxOptions } from '@fairgarden/docs/withFairGardenDocs';
...
- `baseDir` (string, optional): Base directory to strip from absolute file paths before matching. When using `getFairGardenDocsMdxOptions()`, this defaults to `process.cwd()`. When calling the plugin directly, you should provide this for accurate path filtering.
...
- [`docs validate`](../../commands/validate/page.mdx) - CLI command that validates index files
- [`transformMarkdownCode`](../transform-markdown-code/page.mdx) - Transform code blocks in documentation
- [`withFairGardenDocs`](../with-fairgarden-docs/page.mdx) - Next.js plugin with all docs features
