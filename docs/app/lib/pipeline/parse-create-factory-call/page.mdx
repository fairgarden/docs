...
It is the shared primitive that powers all of the FairGarden Docs server loaders ([`loadServerCodeMeta`](../load-server-code-meta/), [`loadServerSitemap`](../load-server-sitemap/), [`loadPrecomputedTypes`](../load-precomputed-types/), and the precomputed code highlighter). It can also be used directly inside a custom [`CodeProvider`](../../components/code-provider/) — for example, to discover the variants of a remote demo from raw text fetched over HTTP.
...
} from '@fairgarden/docs/pipeline/parseCreateFactoryCall';
