...
The plugin is automatically included when using the FairGarden Docs Next.js plugin:

```javascript
// next.config.js
const { withFairGardenDocs } = require('@fairgarden/docs/next');

module.exports = withFairGardenDocs({
  // Your Next.js config
});
```
...
import transformMarkdownRelativePaths from '@fairgarden/docs/pipeline/transformMarkdownRelativePaths';
