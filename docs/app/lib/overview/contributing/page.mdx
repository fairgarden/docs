...
This guide covers how to set up your development environment and contribute to `@fairgarden/docs`.
...
git clone https://github.com/fairgarden/docs.git
cd docs
...
```text
docs/app/lib/{category}/{function-name}/page.mdx
docs/app/lib/{category}/{function-name}/demos/{demo-name}/index.ts
```
...
| `with`      | Configuration plugins         | `withFairGardenDocs`    |
...
5. Document at `docs/app/lib/functions/{function-name}/page.mdx`
...
4. Document at `docs/app/lib/components/{component-name}/page.mdx`

### New webpack Loader

1. Create `src/pipeline/loadPrecomputed{Name}/loadPrecomputed{Name}.ts`
2. Add to `withFairGardenDocs` default patterns if appropriate
...
```bash
# Add a dependency
pnpm -F @fairgarden/docs add lodash

# Remove a dependency
pnpm -F @fairgarden/docs remove lodash
