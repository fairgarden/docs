...
- **Copying again**: State stays `true` while the new write is in flight, so feedback keyed on it doesn't flicker; each successful write restarts the countdown, so the state stays `true` until `timeout` after the last write to finish. A write still in flight `timeout` after it started ends the feedback, so a stalled write can't keep it on. Use `onCopied` to react to every copy.
