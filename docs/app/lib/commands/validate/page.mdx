...
| :------- | :--------- | :-------------------------------------------------- |
| `paths`  | `string[]` | Optional paths to validate (e.g., `lib/components`) |

## Options

| Option                    | Type      | Default                  | Description                                     |
| :------------------------ | :-------- | :----------------------- | :---------------------------------------------- |
| `--command`               | `string`  | `pnpm run docs validate` | Command to suggest when indexes are out of date |
...
```bash
pnpm run docs validate lib/components lib/functions
```
...
```bash
pnpm run docs validate --useVisibleDescription
```
...
```bash
pnpm run docs validate --indexes
```
...
When [`withFairGardenDocs`](../../pipeline/with-fairgarden-docs/page.mdx) is configured with a `requireDemoClient` import specifier, the command also:
...
When [`withFairGardenDocs`](../../pipeline/with-fairgarden-docs/page.mdx) is configured with `requireDemoPage: true`, the command also:
...
Updated index files:
  app/lib/components/page.mdx
  app/lib/functions/page.mdx

Processing 12 types.md files...

Updated types.md files:
  app/lib/components/code-highlighter/types.md
...
✗ Generated files are out of date. Run this command locally:
  ...
  pnpm run docs validate lib/components
...
- Location: `.next/cache/fairgarden-docs/index-updates/`
...
- [`withFairGardenDocs`](../../pipeline/with-fairgarden-docs/page.mdx) - Provides the `requireDemoClient` and `requireDemoPage` options that enable auto-generated demo `client.ts` and `page.tsx` files
